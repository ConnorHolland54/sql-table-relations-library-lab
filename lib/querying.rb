...
def select_name_and_series_subgenres_of_authors
  "SELECT authors.name, subgenres.name FROM authors INNER JOIN series ON authors.id = author_id INNER JOIN subgenres ON subgenre_id = subgenres.id"
end
...
def select_character_names_and_number_of_books_they_are_in
  "SELECT characters.name, COUNT(character_books.character_id) FROM characters INNER JOIN series ON characters.author_id = series.author_id INNER JOIN books ON series.id = books.series_id INNER JOIN character_books ON books.id = character_books.book_id WHERE character_books.character_id = characters.id GROUP BY characters.name ORDER BY COUNT(character_books.character_id) DESC, characters.name ASC"
end
